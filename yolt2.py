...
def init_args():
    ...
    parser.add_argument('--test_labels', type=str, default='car.txt',
    ...
    parser.add_argument('--yolt_dir', type=str,
                        default='/media/ubuntu/45860a09-77fc-4f27-8cf3-5739e384e61d/wujinming/yolt/',
    ...
    if args.mode == 'test':
        # populate labels
        with open(os.path.join(args.data_dir, args.test_labels), 'r+') as fin:
            for l in fin.readlines():
                # spaces in names screws up the argc in yolt2.c
                test_labels_list.append((l[:-1].replace(' ', '_')).encode())
...
def split_valid_im(im_root_with_ext, args):
    ...
    if args.slice_sizes[0] > 0:
        ...
        n_files_str = '"Nu------m files: ' + str(len(valid_files)) + '\n"'
...
def set_valid_files(valid_files_list, args):
    ...
    # valid_files_txt = os.path.join(args.yolt_dir, 'valid_input_files.txt')
...
def run_valid(args):
    ...
    # args.valid_testims_list = args.valid_testims_list[:-1]  # this is dmc added
    for i, valid_base_tmp in enumerate(args.valid_testims_list):
        print(len(args.valid_testims_list))
        print('************')
        print(args.valid_testims_list)
        print('************')
        iter_string = '"\n' + str(i + 1) + ' / ' + \
...
def main():
    ...
    with open(labels_log_file, "w") as fp:
